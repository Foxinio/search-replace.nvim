...
require("search_replace").setup({ search = { debounce = 0, command = { discover } } })
...
wait_for(function()
  return type(picker.manager) == "table" and picker.manager:num_results() == 2 and preview():find(" 1 foo foo", 1, true)
end, "source preview")
...
assert(status.layout and status.picker.manager:num_results() == 2)
require("telescope.actions").close(prompt_bufnr)
